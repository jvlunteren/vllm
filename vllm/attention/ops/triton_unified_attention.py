...
@triton.jit
def find_seq_idx(query_start_len_ptr, target_idx, num_seqs, seq_idx_offset,
                 BLOCK_Q: tl.constexpr, use_q_block_mode: tl.constexpr):
    left: tl.int32 = 0
    right = num_seqs
    while left < right:
        mid = (left + right) // 2
        val = tl.load(query_start_len_ptr + mid + seq_idx_offset) - seq_idx_offset
        mid_val = val // BLOCK_Q + mid if use_q_block_mode else val
        if mid_val <= target_idx:
            left = mid + 1
        else:
            right = mid
    return left - 1 + seq_idx_offset
...
def kernel_unified_attention_2d(
    ...
    seq_idx = find_seq_idx(query_start_len_ptr, q_block_global_idx, num_seqs, seq_idx_offset, BLOCK_Q, True)

    q_block_start_idx = (tl.load(query_start_len_ptr + seq_idx) - seq_idx_offset) // BLOCK_Q + seq_idx - seq_idx_offset
...
def reduce_segments(
    ...
    seq_idx = find_seq_idx(query_start_len_ptr, query_token_idx, num_seqs, seq_idx_offset, BLOCK_Q, False)
...
def unified_attention(
    ...
    BLOCK_M_PREFILL = 64
    BLOCK_Q_PREFILL = BLOCK_M_PREFILL // num_queries_per_kv

    TILE_SIZE_DECODE = 32
    BLOCK_M_DECODE = 16
    BLOCK_Q_DECODE = BLOCK_M_DECODE // num_queries_per_kv
    ...
    total_num_q_blocks_prefill = (q.shape[0] - num_decodes) // BLOCK_Q_PREFILL + num_seqs - num_decodes
    total_num_q_blocks_decode = num_decodes // BLOCK_Q_DECODE + num_decodes

    # prefill
    if num_seqs > num_decodes:
        kernel_unified_attention_2d[(
            total_num_q_blocks_prefill,
            num_kv_heads,
        )](
            output_ptr=out,
            query_ptr=q,
    ...
    if num_decodes > 0:
        ...
        if True: #total_num_q_blocks_decode * num_kv_heads > 128:
            kernel_unified_attention_2d[(
                total_num_q_blocks_decode,
                num_kv_heads,
            )](
                output_ptr=out,
                query_ptr=q,
